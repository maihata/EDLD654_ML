...
```{r}
### 11.8.25. THIS IS DIFFERENT FROM FP.qmd ### 
...
autism_final <- autism_final %>%
  select(-starts_with("Class.ASD"))
```
...
```{r}
# 1) Rebuild features: drop A1–A10 and Class.ASD* but KEEP everything else
library(dplyr)

X_tr <- autism_tr %>%
  select(-matches("^A(10|[1-9])_Score$"), -starts_with("Class.ASD")) %>%
  select(-score)
...
# 4) 10-fold CV with caret (GLM Gaussian = OLS)
library(caret)
set.seed(20251102)
...
glm_cv$results  # averaged CV metrics
...
cat("Held-out Test (caret GLM, 10-fold CV)\n",
    "R²:", round(R2_cv, 3),
    " MAE:", round(MAE_cv, 3),
...
```{r}
### PUT THIS BACK IF IT DOESN't WORK to drop the A1-A10 AND ASD.Class_NO 
# library(dplyr)
# 
...
length(coef(glm_cv$finalModel))         # caret GLM (final model)
...
-   **X-axis (λ):** how strong the penalty is (bigger = stronger shrinkage).

    **Color (α):** mixing parameter (0 = ridge, 1 = lasso).

    **Y-axis (RMSE):** smaller is better — you’ll see the “valley” where performance was best
