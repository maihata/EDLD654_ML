...
blueprint <- recipe(score ~ ., data = autism_tr) %>%
  step_rm(matches("^A(10|[1-9])_Score$")) %>%      # remove A1–A10 scores
  step_zv(all_numeric()) %>%
...
# Rank coefficients by absolute size (exclude intercept)
...
```{r}
# ✅ Chunk 7 – Evaluate model on test set
...
```{r}
# ✅ Chunk 8 – Save the trained model (optional)

# Keep only ridge object and save environment
rm(list = ls()[!ls() %in% c("ridge")])
