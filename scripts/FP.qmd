...
ggplot2::ggplot(coef_df, ggplot2::aes(x = reorder(term, abs_beta), y = beta)) +
  ggplot2::geom_col() +
  ggplot2::coord_flip() +
  ggplot2::labs(title = "Top 15 predictors by |β| (Ridge)",
...
lasso_grid <- expand.grid(
  alpha  = 1,                                # Lasso
  lambda = 10 ^ seq(3, -4, length.out = 80)  # wide log grid
)

lasso <- caret::train(
  score ~ .,
  data        = autism_tr_r,   # reuse frames from build-ridge-frames
  method      = "glmnet",
  trControl   = cv10,
  tuneGrid    = lasso_grid,
  standardize = FALSE          # already centered/scaled upstream
)
...
#| label: eval-lasso
#| dependson: fit-lasso
pred_lasso <- predict(lasso, newdata = autism_te_r)
...
-   Among all models, Lasso regression showed the best performance (R2 = 0.317) than Ridge and Elastic Net did. All three models did significantly better than the mean-only baseline did.
