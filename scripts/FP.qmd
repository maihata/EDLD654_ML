...
format:
  revealjs:
    theme: serif
    footer: "Maiko Hata - https://maihata.github.io/finalproj/"
    width: 960
    height: 700
    margin: 0.08
editor: visual
...
-   ... and yet, I initially included 10 screener questions (A1_Score\~A10_Score) and "Class.ASD" , overlooking that "result" which is the total of A1\~A10 was already my outcome variable, and "Class.ASD" was simply coded “YES” for scores of 6 or higher.
...
```{r}
#| include: false
# ✅ 10-fold CV OLS (caret), then test
library(caret)
set.seed(20251102)
...
Even though I tuned alpha from 0 to 1, the Elastic Net model chose **alpha = 0**, meaning the best-performing model ended up acting like Ridge. In my dataset, with many correlated demographic features, this gentle ridge-style shrinkage improved accuracy more than LASSO’s stronger, variable-dropping penalty. As a result, Ridge and Elastic Net showed the strongest performance (R² = 0.309), while LASSO performed slightly lower (R² = 0.281). Still, all three regularization models performed far better than the mean-only baseline (R² = –0.004).
...
-   The AQ was developed using adults in the UK who spoke English, of unknown race, and with men overrepresented (Baron-Cohen et al., 2001). This makes the norming process quite skewed.
-   I struggled with the ethical implications of applying ML. It is unclear whether participants consented to secondary use of their responses. Conducting these models requires not only methodological care but also ethical reflection about data ownership, informed consent, and the risks of misinterpretation.

![](images/whosedata.jpeg){fig-align="center" width="60%"}
...
![](images/arigato_bubbles.png){fig-align="center" width="40%"}
